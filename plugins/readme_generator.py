...
class ReadmeGenerator:
    ...
    # Side keywords used in render filenames (<board>_<side>.png).
    _SIDES = ['top', 'bottom', 'left', 'right', 'front', 'back']

    @classmethod
    def _side_caption(cls, image_path):
        """Derives a 'Top'/'Bottom'/... caption from a render filename, or None."""
    ...
    def update_readme(self, kicad_version="Unknown KiCad Version", board_images=None,
        ...
        # If no fresh render was passed but image rendering is enabled, keep any
        # existing rendered image(s) embedded (a skipped/failed render this commit
        # shouldn't drop the image from the README).
        if not board_images and self.settings.get('render_image', False) and pcb_file:
            base = os.path.splitext(os.path.basename(pcb_file))[0]
            sides = ['top', 'bottom'] if self.settings.get('render_both_sides', False) \
                else [self.settings.get('render_side', 'top')]
            found = []
            for side in sides:
        ...
        if sch_files:
            ...
            for ref, data in bom.items():
                ...
                if is_ic:
                    ...
                    c_type = 'Battery' if ref.startswith('BT') else 'Switch' if ref.startswith('SW') or ref.startswith('S') else 'Connector'
        ...
        # --- 3. Build Markdown ---
        md = ["<!-- KICAD_DIFF_GEN_START -->"]

        # Centered board render(s) with a side caption (HTML tag: GitHub markdown
        # strips CSS, so <p align> is the reliable way to center images).
        if board_images:
            img_w = self.settings.get('readme_image_width', 500)
            valid = [img for img in board_images
